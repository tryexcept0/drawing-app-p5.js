	...
	this.draw = function() {
		...
		if (mouseIsPressed) {
			...
			//draw a line between them and the current positions
				...
				layers[activeLayer].gfx.line(previousOppositeMouseX, previousOppositeMouseY, oX, oY);
		...
		layers[activeLayer].gfx.strokeWeight(3);
		layers[activeLayer].gfx.stroke("red");
		//draw the line of symmetry
		if (this.axis == "x") {
			layers[activeLayer].gfx.line(width / 2, 0, width / 2, height);
		} else {
			layers[activeLayer].gfx.line(0, height / 2, width, height / 2);
		}
